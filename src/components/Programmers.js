...
export default function Programmers() {
  ...
  const [programmerName, selectProgrammer] = useState(listOfAwesome);
  const [programmerId, selectId] = useState(null);
  ...
  const getNameOfFeatured = () => 
    ...
    programmerName[programmerId-1].name;
  ...
  const style = {
    ...
    color:  programmerId ? "gold" : "royalblue",
  };
      ...
      <div className='programmers'>
        {
          ...
          programmerName.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { selectId(dev.id)/* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
